...
- **QR 촬영 확인**: 모바일/웹 카메라를 통한 QR 스캔으로 자산 실사 (웹 카메라 실패 시 수동 입력 폴백)
- **실사 사인**: 실사 담당자 친필 서명 기능으로 책임 소재 명확화
- **전사 실사 라운드(차수) 관리**: 관리자 그룹이 년도별 실사 차수 생성/시작/종료, 라운드 활성 시에만 촬영 가능
- **역할 기반 권한**: admin(관리자), operator1/2(운영자), user(사용자) — 관리자 그룹만 차수 관리 및 완료건 수정
- **실사 기록 저장**: 실사 일시, 담당자, 자산 상태, 서명 이미지 등 이력 관리, 고아파일 자동 삭제
...
#### 5) 에이전트 실시간 연동
- **Postgres Changes 구독**: 에이전트 Heartbeat로 변경된 `assets` 데이터를 폴링 없이 즉시 반영
- **에이전트 접속 상태 (Presence)**: 에이전트 WebSocket 접속 여부를 실시간 추적 → 이중 인디케이터 표시
- **관리자 명령 전송 (Broadcast)**: 특정 기기에 "즉시 Heartbeat", "시스템 정보 갱신" 명령 전송
- **에이전트 알림 수신 (Broadcast)**: 배터리 부족, 네트워크 변경 등 긴급 알림 SnackBar 표시
- **사용자 확인 상태**: 에이전트 사용자 확인 결과 (`verified`/`mismatch`) 자산 목록/상세에 표시
- **자산 배정 수령 상태**: 배정 수령 확인 결과 (`pending`/`confirmed`) 자산 목록/상세에 배지 표시
...
| `google_mlkit_text_recognition` | ^0.13.0 | 디바이스 라벨 OCR (모바일, 온디바이스) |
...
├── theme.dart                     # 라이트/다크 테마 설정 (Material 3)
├── models/                        # 데이터 모델
│   ├── asset.dart                 # 자산 모델
│   ├── user.dart                  # 사용자 모델
...
│   ├── agent_presence_notifier.dart  # 에이전트 Presence 상태 관리
│   └── agent_alert_notifier.dart     # 에이전트 Broadcast 알림 관리
├── screens/                       # 화면 UI
...
└── services/                      # API 통신 레이어
    ├── api_service.dart           # Supabase 기반 API 서비스 (PostgREST)
    ├── auth_service.dart          # 로그인/토큰 갱신/로그아웃 API
    ├── realtime_service.dart      # Supabase Realtime 구독 관리 (Postgres Changes, Broadcast, Presence)
...
- **행 클릭 → 자산 상세 페이지 이동** (`/asset/:id`)
  ...
  - 인라인 편집 모드 전환 가능
- **필터**:
  - 상단 필터바: 유형별, 상태별 드롭다운 필터
  - 정렬: id 내림차순 고정
- **검색**: 자산번호, 자산명, 시리얼번호, 사용자명으로 즉시 검색
...
1. 스캔 페이지 진입 → 카메라 권한 확인
   - **모바일**: `MobileScanner` 카메라 QR 실시간 인식
   - **웹**: `MobileScanner` 웹 지원 (mobile_scanner 5.x), 카메라 접근 실패 시 **자산번호 수동 입력 폴백**
2. QR 코드 스캔 → `asset_uid` 추출
   - 미등록 자산 → 등록 다이얼로그 → `/asset/new` (자산번호 자동 입력)
3. 서버에서 자산 정보 조회
4. 실사 데이터 생성:
   - 위치 정보 입력 (건물, 층수, 자리번호)
   - 자산 상태 선택
   - **실사 사진 촬영**: 자산 현물 사진 촬영/첨부 (용량 최적화: 1280x960, JPEG 70%)
   - **친필 서명**: Signature Pad로 담당자 서명 입력 (펜 두께 3px)
   - 서명 이미지 자동 저장 (PNG 포맷)
5. 서버 동기화 (온라인) 또는 로컬 저장 (오프라인)
6. **사진 저장 경로**: `{round}/photos/{assetCode}_{page}_{timestamp}_{round}.jpg`
   - Storage 업로드 성공 → DB 실패 시 **고아파일 자동 삭제**
   - 임시 파일 자동 정리 (`TempFileCleaner`)

#### 4.2.2 전사 실사 라운드(차수) 관리
> 관리자 그룹(admin, operator1, operator2)이 년도별 실사 차수를 생성/시작/종료

- **라운드 상태 흐름**: `draft`(대기) → `active`(진행중) → `closed`(종료)
- **동시 진행 제한**: active 라운드는 1개만 허용
- **권한**: 관리자 그룹만 생성/시작/종료 가능, 일반 사용자는 조회만
- **실사 연동**: 라운드가 active일 때만 일반 사용자가 사진/서명 등록 가능 (관리자 그룹은 항상 가능)
- **사진 저장 경로**: 라운드 번호가 경로에 포함 (`{round}/photos/...`)

#### 4.2.3 실사 기록 목록 (리스트)
> 실사 이력을 리스트 형식으로 조회하는 화면

- **활성 라운드 배너**: 상단에 현재 진행 중인 실사 라운드 표시 (없으면 안내 메시지)
- **차수 관리 버튼**: 관리자 그룹에만 앱바에 설정 아이콘 표시 → 라운드 관리 다이얼로그
- **리스트 구조**:
...
| 담당자 | `inspector_name` (실사 담당자) |
| 실사일 | `inspection_date` |
| 회차 | `inspection_count` (n회차) |
| 완료여부 | 완료/미완료 뱃지 |

  - **30개 항목 단위 페이지네이션**
- **행 클릭 → 실사 상세 페이지 이동**
- **필터**: 상태 (전체/완료/미완료), 검색 (자산번호/담당자)

#### 4.2.4 실사 상세/권한/초기화
- **실사 상세 화면**: 사용자명, 사용자부서, 자산번호, 자산위치, 실사사진, 친필서명 표시
- **라운드 정보 배너**: 활성 라운드명 표시, 라운드 없으면 "촬영 불가" 안내
- **사진 촬영**: 실사 상세에서 직접 촬영 가능 (카메라 → Storage 업로드 → DB 업데이트)
- **연결 동선**: 실사 목록에서 실사 자산 선택 → 실사 상세(`/inspection/:id`) 이동
- **자산 이동**: 실사 상세에서 [자산 상세 보기] → 자산 상세(`/asset/:id`) 이동
- **완료 조건**: 5개 필드 모두 NOT NULL (건물, 층, 위치, 사진, 서명)
- **수정 권한**:
  - 일반사용자: 완료된 실사 수정 불가 + 라운드가 active일 때만 사진/서명 등록 가능
  - 관리자 그룹: 완료 여부/라운드 상태와 무관하게 항상 수정 가능
- **실사초기화**: 관리자만 실행 가능, 상태/사진/서명/메모를 초기화 후 재실사 상태로 전환
- **에러 메시지**: 모든 에러 SnackBar는 `SelectableText`로 표시 (복사 가능)
...
- **필터링**: 건물별, 부서별, 상태별, 지급형태별, 서명 유무별 필터, **사용자 확인 상태별** 필터 (확인완료/불일치/미확인)
- **검색**: 자산 코드, 담당자 이름으로 검색
- **통계**: 건물별/부서별 실사 진행률 차트

### 4.6 에이전트 Realtime 연동

#### 4.6.1 RealtimeService (`lib/services/realtime_service.dart`)
앱 시작(인증 후) 시 모든 Realtime 채널을 구독하고, 로그아웃 시 구독을 해제합니다.

| 타이밍 | 동작 |
|--------|------|
| 인증 성공 후 | `RealtimeService.subscribeAll()` — 모든 채널 구독 시작 |
| 로그아웃 시 | `RealtimeService.unsubscribeAll()` — 모든 채널 구독 해제 |

#### 4.6.2 Postgres Changes 구독
에이전트 Heartbeat로 변경된 `assets` 테이블 데이터를 폴링 없이 즉시 UI에 반영합니다.

**자동 갱신 대상 컬럼:**

| 컬럼 | 갱신 시점 | 프론트엔드 반영 |
|------|----------|---------------|
| `last_active_at` | Heartbeat | 접속현황 인디케이터 즉시 갱신 |
| `specifications.device_status` | Heartbeat | 시스템 정보 표시 즉시 갱신 |
| `verification_status` | `verify_user` RPC | 사용자 확인 상태 아이콘 즉시 갱신 |
| `last_verified_at` | `verify_user` RPC | 마지막 확인 일시 즉시 갱신 |
| `assignment_status` | `confirm_assignment` RPC | 배정 수령 상태 배지 즉시 갱신 |
| `assignment_confirmed_at` | `confirm_assignment` RPC | 수령 확인 일시 즉시 갱신 |

```dart
// RealtimeService 내부
final channel = supabase.channel('assets-realtime')
  .onPostgresChanges(
    event: PostgresChangeEvent.update,
    schema: 'public',
    table: 'assets',
    callback: (payload) {
      ref.read(assetNotifierProvider.notifier).updateLocal(
        Asset.fromJson(payload.newRecord),
      );
    },
  )
  .subscribe();
...
#### 4.6.3 에이전트 Presence 구독 (접속 상태)
`agent-presence:global` 채널을 구독하여 에이전트의 실시간 WebSocket 접속 여부를 추적합니다.
...
Presence (실시간 WebSocket 접속)와 Heartbeat (`last_active_at`) 두 가지 데이터를 조합하여 접속 상태를 표시합니다.

| 조건 | 인디케이터 | 색상 | 설명 |
|------|-----------|------|------|
| Presence 연결 + Heartbeat 최신 | 🔵 실시간 연결됨 | `#2196F3` (Blue) | WebSocket 연결 활성 상태 |
| Presence 미연결 + Heartbeat 최신 (< 60분) | 🟢 접속중 | `#4CAF50` (Green) | 기존 Heartbeat 기반 접속중 (6.4 참고) |
| Presence 미연결 + Heartbeat 경과 1~31일 | 🟡 N일전 | `#8BC34A` (Light Green) | 기존 인디케이터 (6.4 참고) |
| Presence 미연결 + Heartbeat 만료 | 🔴 만료 | `#F44336` (Red) | 기존 인디케이터 (6.4 참고) |
| Presence 미연결 + 미접속 | ⚪ 미접속 | `#9E9E9E` (Grey) | 한번도 접속 안 함 |

> **우선순위**: Presence 연결 상태가 Heartbeat 기반 인디케이터보다 우선합니다.
> Presence 연결 해제 시 기존 Heartbeat 기반 인디케이터로 자동 전환됩니다.
...
```dart
// 자산 상세 페이지 버튼 핸들러
Future<void> requestHeartbeat(String assetUid) async {
  final channel = supabase.channel('agent-commands:$assetUid');
...
```dart
final alertChannel = supabase.channel('agent-alerts:global')
  .onBroadcast(
    event: 'alert',
    callback: (payload) {
      ref.read(agentAlertNotifierProvider.notifier).addAlert(payload);
      // SnackBar 또는 Toast로 즉시 표시
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('${payload['asset_uid']}: ${payload['message']}')),
      );
    },
  )
  .subscribe();
...
| alert_type | SnackBar 표시 |
|------------|--------------|
| `battery_low` | "BDT00001: 배터리 15% 이하" (경고색) |
| `network_changed` | "BDT00001: 네트워크 변경됨" (정보색) |
| `agent_started` | "BDT00001: 에이전트 시작됨" (성공색) |
| `agent_stopped` | "BDT00001: 에이전트 종료됨" (경고색) |

#### 4.6.7 사용자 확인 상태 표시

에이전트에서 `verify_user` RPC 호출 → `assets.verification_status` 갱신 → Postgres Changes로 프론트엔드에 즉시 반영.

**자산 목록 페이지:**

| verification_status | 표시 | 설명 |
|---------------------|------|------|
| `NULL` (미확인) | 회색 아이콘 | 사용자 확인 미실시 |
| `verified` | 초록색 체크 ✓ | 사용자 일치 확인 완료 |
| `mismatch` | 빨간색 경고 ⚠ | 사용자 불일치 → 관리자 확인 필요 |

- `mismatch` 자산은 목록 상단에 경고 배지 표시
- 필터: "사용자 불일치" 필터 옵션 추가 (`verification_status = 'mismatch'`)

**자산 상세 페이지 — 사용자 확인 현황 섹션:**
```
── 사용자 확인 현황 ──────────
확인 상태: ✓ 확인 완료 (또는 ⚠ 불일치)
마지막 확인: 2026-03-14 10:30
확인 주기: 30일 (다음 확인: 2026-04-13)
```

- 불일치 시 경고 카드: "기존 사용자와 다른 사용자입니다. 확인이 필요합니다."
- `last_verified_at` + `access_settings.verification_interval_days` (기본 30일)로 다음 확인 예정일 계산

#### 4.6.8 자산 배정 수령 상태 표시

에이전트에서 `confirm_assignment` RPC 호출 → `assets.assignment_status` 갱신 → Postgres Changes로 프론트엔드에 즉시 반영.
...
| 스캔 | `/scan` | QR 스캔 (모바일: 카메라, 웹: 카메라+수동입력 폴백, 최대 5건) |
| 자산 목록 | `/assets` | 자산 목록 (컴팩트 필터바, 인라인 등록 버튼, 컬럼 설정) |
| 자산 상세 | `/asset/:id` | 자산 상세/편집 (OCR 라벨 인식, 실시간 OCR 스캔, 실사 등록) |
| 자산 등록 | `/asset/new` | 신규 자산 등록 (QR 스캔 → 자산번호 자동 입력, 자산 상세와 동일 위젯) |
| 실사 목록 | `/inspections` | 실사 목록 (라운드 배너, 관리자 차수 관리, 상태 필터) |
| 실사 상세 | `/inspection/:id` | 실사 상세 (사진촬영, 서명, 라운드 권한 제어, 사진/서명 레이지 로딩) |
...
| 영역 | 내용 |
|------|------|
| 상단 | 검색창 (자산번호/자산명/시리얼번호/사용자명) + **검색** 버튼 |
| 필터바 | 유형별, 상태별 드롭다운 필터 |
| 열 설정 | **열 표시 설정** 버튼 (컬럼 표시/숨김 및 순서 변경) |
| 리스트 | 자산 리스트 (6.6 리스트 목록 스타일 적용) |
| 하단 | 페이지네이션 (이전/다음 버튼, 현재/전체 페이지 표시) |

- **30건 단위** 페이지네이션
- 전체 33개 컬럼 표시 (4.1.1 컬럼 매핑 테이블 참고), 열 표시 설정으로 컬럼 표시/숨김 전환 가능
...
### 6.4 접속현황 인디케이터 색상
> 사용자의 **최종 접속 시점** 및 **에이전트 Presence 상태**를 기준으로 접속 상태를 시각적으로 표시합니다.
> Presence 연결 상태가 우선하며, 미연결 시 `assets.last_active_at` 값과 `access_settings` 테이블의 임계값을 비교하여 결정합니다.

| 상태 | 조건 | Light Mode | Dark Mode | 표시 |
|------|------|-----------|-----------|------|
| 실시간 연결 | Presence 채널에 연결됨 | `#2196F3` (Blue) | `#64B5F6` (Blue 300) | 파란색 동그라미 (숫자 없음) |
| Heartbeat 접속 | Presence 미연결 + `last_active_at`이 `active_threshold_minutes` 이내 (기본 60분) | `#4CAF50` (Green) | `#81C784` (Green 300) | 초록색 동그라미 (숫자 없음) |
| 경과 1~31일 | Presence 미연결 + 경과일 1 ~ `warning_threshold_days` (기본 31일) | `#8BC34A` (Light Green) | `#AED581` (Light Green 300) | 연두색 동그라미 + 경과일 숫자 |
| 접속 만료 | Presence 미연결 + 경과일 > `warning_threshold_days` | `#F44336` (Red) | `#E57373` (Red 300) | 빨간색 동그라미 (숫자 없음) |
| 미접속 | Presence 미연결 + `last_active_at`이 NULL (한번도 접속 안 함) | `#9E9E9E` (Grey) | `#757575` (Grey 600) | 회색 동그라미 (숫자 없음) |
...
> **Presence 우선**: Presence 연결 상태가 확인되면 Heartbeat 경과 시간과 무관하게 "실시간 연결" (파란색)으로 표시합니다. 상세 사항은 4.6.4 이중 접속 상태 인디케이터를 참고합니다.

#### 인디케이터 위젯 사양
- **크기**: 16×16dp (리스트 내), 24×24dp (상세 화면)
- **형태**: 원형 (`CircleAvatar` 또는 `Container` with `BoxShape.circle`)
- **경과일 숫자**: 원 내부 중앙 정렬, 폰트 크기 8sp (16dp) / 11sp (24dp), 검정색 텍스트
- **사용 위치**: 자산 목록의 사용자 컬럼 옆, 사용자 목록, 홈 대시보드

```
 예시 (라이트 모드)
 ●        ← 파란색: 실시간 연결됨 (WebSocket Presence 활성)
 ●        ← 초록색: Heartbeat 접속중 (1시간 이내, Presence 미연결)
 ❶        ← 연두색 + "1": 1일 경과
 ❷        ← 연두색 + "2": 2일 경과
  ...
 ㉛       ← 연두색 + "31": 31일 경과
...
| **List Item** | **14sp** | **리스트 목록 행 텍스트 (자산 목록, 실사 목록 등) — bodyMedium** |

### 6.6 리스트 목록 스타일
> 자산 목록, 실사 목록 등 데이터 리스트 테이블에 적용되는 공통 스타일입니다.

- **행 텍스트 크기**: 14sp (bodyMedium)
- **세로 구분선**: dividerColor (셀 사이 세로줄 표시)
- **가로 구분선**: 행 사이 가로줄 표시 (dividerColor, 0.55 투명도)
...
| GET | `/api/inspection_rounds` | 실사 라운드 목록 조회 |
| GET | `/api/inspection_rounds?status=active` | 활성 라운드 조회 |
| POST | `/api/inspection_rounds` | 라운드 생성 (관리자 그룹) |
| RPC | `start_inspection_round(p_round_id)` | 라운드 시작 draft→active (관리자 그룹) |
| RPC | `close_inspection_round(p_round_id)` | 라운드 종료 active→closed (관리자 그룹) |
...
| `last_active_at` | TIMESTAMPTZ | 최종 접속 시각 (접속현황 인디케이터 기준, 6.4 참고) |
| `last_verified_at` | TIMESTAMPTZ | 에이전트 사용자 확인 최종 시각 (4.6.7 참고) |
| `verification_status` | TEXT | 사용자 확인 상태 (`verified`/`mismatch`/NULL, 4.6.7 참고) |
| `assignment_status` | TEXT | 배정 수령 상태 (`pending`/`confirmed`/NULL, 4.6.8 참고) |
| `assignment_confirmed_at` | TIMESTAMPTZ | 자산 수령 확인 시각 (4.6.8 참고) |
...
| `auth_uid` | TEXT | Supabase Auth UID (auth.users.id 참조) |
| `auth_provider` | TEXT | 인증 제공자 (email / kakao / google) |
| `sns_id` | TEXT | SNS 로그인 시 외부 서비스 사용자 ID |
| `role` | TEXT | 사용자 역할 (`admin`, `operator1`, `operator2`, `user`) 기본값: `user` |
| `created_at` | TIMESTAMPTZ | 생성일 |
| `updated_at` | TIMESTAMPTZ | 수정일 |

> **역할 체계**:
> - **관리자 그룹**: `admin`(관리자), `operator1`(운영자1), `operator2`(운영자2) → 차수 관리, 완료건 수정 가능
> - **사용자 그룹**: `user` → 활성 라운드 시에만 실사 수정 가능
> - Dart 모델: `User.isAdminGroup` getter로 관리자 그룹 판별
>
> **참고**: 비밀번호(password_hash)는 백엔드에서만 관리하며, 프론트엔드에는 노출되지 않습니다.
...
| `round_id` | BIGINT | 전사 실사 라운드 FK (inspection_rounds.id 참조) |
| `synced` | BOOLEAN | 서버 동기화 여부 (기본값: true) |
| `created_at` | TIMESTAMPTZ | 생성일 |
| `updated_at` | TIMESTAMPTZ | 수정일 (트리거 자동 갱신) |

> **완료 판정 기준**: `inspection_building`, `inspection_floor`, `inspection_position`, `inspection_photo`, `signature_image` 5개 필드가 모두 NOT NULL이면 완료(completed)로 간주합니다.
>
> **사진 저장 경로 패턴**: `{round}/photos/{assetCode}_{page}_{timestamp}_{round}.jpg`
> - `page`: `detail`(실사상세), `register`(실사등록)
> - 서명: `{round}/signatures/{inspId}_sig_{timestamp}_{round}.png`
> - 버킷: `inspection-photos` (사진), `inspection-signatures` (서명)

### 8.5 inspection_rounds (전사 실사 라운드)
| 컬럼 | 타입 | 설명 |
| --- | --- | --- |
| `id` | BIGINT | 라운드 기본 키 (자동 증가) |
| `year` | INTEGER | 실사 년도 |
| `round` | INTEGER | 차수 (1, 2, 3...) |
| `title` | TEXT | 라운드 제목 (예: "2026년 1차 실사") |
| `status` | TEXT | 상태 (`draft`, `active`, `closed`) |
| `started_by` | BIGINT | 시작한 사용자 FK |
| `started_at` | TIMESTAMPTZ | 시작 일시 |
| `closed_by` | BIGINT | 종료한 사용자 FK |
| `closed_at` | TIMESTAMPTZ | 종료 일시 |
| `created_at` | TIMESTAMPTZ | 생성일 |
| `updated_at` | TIMESTAMPTZ | 수정일 |

> **제약**: `(year, round)` UNIQUE — 같은 년도에 같은 차수 중복 불가
> **RLS**: 조회는 모든 인증 사용자, 생성/수정은 관리자 그룹만, 삭제는 관리자만
> **RPC**: `start_inspection_round(p_round_id)` (draft→active), `close_inspection_round(p_round_id)` (active→closed)
...
### 9.8 AgentAlertNotifier
```dart
// 에이전트 Broadcast 알림 관리 Notifier
@riverpod
class AgentAlertNotifier extends _$AgentAlertNotifier {
  @override
...
class RealtimeService {
  ...
  Future<void> subscribeAll() async {
    _subscribePostgresChanges();   // assets, asset_inspections
    _subscribePresence();           // agent-presence:global
    _subscribeAlerts();             // agent-alerts:global
  }

  // 로그아웃 시 전체 채널 구독 해제
  Future<void> unsubscribeAll() async { ... }

  // 특정 기기에 명령 전송 (Broadcast)
  Future<void> sendCommand(String assetUid, String command, {Map<String, dynamic>? params}) async { ... }
}
```
...
- [ ] 실사 검색 (자산 UID/메모)
- [ ] 엑셀 내보내기 기능
- [x] ~~**서명 필기 인식 (OCR)**~~ → 디바이스 라벨 OCR로 구현 (모바일: ML Kit, 웹: Tesseract.js)
...
### 19.2 성능 최적화
- [x] ~~이미지 압축 및 캐싱~~ → 실사 사진 촬영 시 용량 최적화 (1280x960, 품질 70%), 서명 export 실제 영역만 PNG, 사진/서명 레이지 로딩 (ExpansionTile)
