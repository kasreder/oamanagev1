...
| 리스트 | 실사 기록 리스트 (6.5 리스트 목록 스타일 적용) |
| 하단 | 페이지네이션 (페이지 번호 버튼, 이전/다음) |
...
- **친필서명**: O / X 아이콘으로 유무만 표시
- **실사사진**: O / X 아이콘으로 유무만 표시
- 행 클릭 → 자산 상세(`/asset/:id`) 이동 (해당 자산의 전체 정보 확인)

#### 5.1.5 자산 상세 (`/asset/:id`)
> 자산번호 기준으로 해당 자산의 모든 정보를 상세 표시

| 영역 | 내용 |
|------|------|
| 상단 | 자산번호(asset_uid) + 상태 뱃지 + 편집/삭제 버튼 |
| 공통 정보 | 자산명, 유형, 시리얼번호, 모델명, 제조사, 건물/층, 지급형태, 담당자 |
...
- **QR 스캔 → 보유 자산인 경우** (asset_uid가 DB에 존재):
  ...
  - **[실사 등록]** 버튼 → 실사 데이터 입력 화면 (위치/상태/사진/서명)
- **QR 스캔 → 보유 자산이 아닌 경우** (asset_uid가 DB에 없음):
  - "등록되지 않은 자산입니다" 안내
  - **[자산 등록]** 버튼 → 자산 등록 화면 (스캔된 코드 자동 입력)
...
| 메인 | 선택한 건물/층의 도면 이미지 + 격자 오버레이 + 자산 마커 |
| 관리 버튼 | 도면 추가/수정/삭제 버튼 (권한에 따라 표시) |

- 건물/층 선택 → 해당 도면 로드
- 도면 이미지가 **매칭되지 않은 경우** (등록된 도면 없음): **"준비중"** 문구 표시
- 도면이 있는 경우: 격자 + 자산 마커 표시 (4.4.2 도면 뷰어 기능 적용)
- 마커 클릭 → 자산 정보 팝업 → 자산 상세 이동 가능
